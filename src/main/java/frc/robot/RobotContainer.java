...
//    rightButtons[1].whileHeld(new AlignToBall(m_driveTrain, m_vision, () -> leftJoystick.getRawAxis(1))); //Bottom (right) Button - Turn to powercells (Automated vision targeting
        ...
        xBoxButtons[0].whileHeld(new EjectAll(m_indexer, m_intake));                          // A - Manual Shot
        xBoxButtons[1].whileHeld(new RapidFire(m_shooter, m_indexer, m_intake, 3500));  // B - Manual Shot - Initiation Line
        xBoxButtons[2].whileHeld(new RapidFire(m_shooter, m_indexer, m_intake, 3700));  // X - Manual Shot - Trench
...
//    xBoxButtons[7].whenPressed(new ToggleIndexerControlMode(m_indexer)); //select - toggle control mode uptake
        ...
        xBoxButtons[9].whenPressed(new EnableClimbMode(m_climber, m_turret)); //right stick
